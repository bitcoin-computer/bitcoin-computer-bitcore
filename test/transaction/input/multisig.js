...
describe('MultiSigInput', () => {
  ...
  it('can parse list of signature buffers, from TX signed with key 1 and 2', () => {
    ...
    inputObj.signatures = MultiSigInput.normalizeSignatures(
      ...
      0,
      transaction.inputs[0].script.chunks.slice(1).map((s) => s.buf),
    ...
    transaction.inputs[0].isValidSignature(transaction, transaction.inputs[0].signatures[0]).should
      .be.true
  ...
  it('can parse list of signature buffers, from TX signed with key 3 and 1', () => {
    ...
    transaction.inputs[0].isValidSignature(transaction, transaction.inputs[0].signatures[0]).should
      .be.true
    transaction.inputs[0].isValidSignature(transaction, transaction.inputs[0].signatures[2]).should
      .be.true
  })
})
